...
const index = async (req, res) => {
    try {
        const types = await Type.find({ deletedAt: null });  // Retrieve all types with deletedAt set to null
...
const update = async (req, res) => {
    const uploadedFile = await uploadFileFunction(req, res, 'image', 'types_images');  // Upload the file if provided
    const identifier = req.params.identifier;
...
const destroy = async (req, res) => {
    ...
    await type.softDelete(req.user._id);  // Soft-delete the type by updating the deletedAt field
    res.json({
        status: 200,
